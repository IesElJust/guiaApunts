...
En aquest document, anem a veure com podríem realitzar la correcció ortogràfica amb Visual Studio Code i l'eina LanguageTool.
...
![L'extensió LTeX](img/ltexExtension.png)
...
Aquesta extensió, per tal de no alentir l'editor, no realitza contínuament correccions, sinó que ho fa cada cert temps (aquesta freqüència es pot modificar a través del paràmetre `LTeX: Check Frequency`, encara que tampoc és recomanable, per no sobrecarregar l'editor).
...
2. Descomprimim el fitxer descarregat en la nostra carpeta d'usuari, per exemple, el que ens crearà la carpeta *LanguageTool-6.6*.
...
1. Ara, per tal d'executar el servidor de LanguageTool, necessitarem l'entorn d'execució de Java (JRE). Si el tenim instal·lat al nostre equip, només caldrà situar-nos dins la carpeta del *LanguageTool-6.6*:
...
2. I llançar-lo amb la següent ordre:
    ...
    ```bash
    2025-10-03 18:08:35.010 GMT+02:00 INFO  org.languagetool.server.DatabaseAccessOpenSource Not setting up database access, dbDriver is not configured
    2025-10-03 16:08:35 +0000 Setting up thread pool with 10 threads
    2025-10-03 18:08:40.615 GMT+02:00 WARN  org.languagetool.language.identifier.DefaultLanguageIdentifier fastText not configured - language detection performance will be degraded. See https://dev.languagetool.org/http-server#starting-from-command-line for instructions.
    2025-10-03 18:08:40.634 GMT+02:00 INFO  org.languagetool.server.TextChecker A/B-Test enabled: []
    2025-10-03 16:08:40 +0000 Starting LanguageTool 6.6 (build date: 2025-03-27 20:50:25 +0100, f3e8d91) server on http://localhost:8081...
    2025-10-03 16:08:40 +0000 Server started
    ```

    **Aneu amb compte i no tanqueu aquesta finestra, ja que quan ho feu, aturareu el servidor.**


!!! note "Si no tenim el JRE..."
    El JRE (Java Runrime Environment), o entorn d'execució de Java, que ofereix junt amb certes llibreries del sistema, la màquina virtual de Java, que és la que s'encarrega d'executar aquest tipus d'aplicacions.
    
    LliureX ja disposa del JRE preinstal·lat per omissió. En cas que no el tinguem instal·lat al nostre equip (no es troba l'ordre `java`), podem instal·lar el paquet `default-jre`, des de qualsevol gestor de programari.
...
![Configuració de les preferències](img/prefsVSCode.png)

Açò ens mostrarà tots els paràmetres de configuració. Podem buscar el que desitgem a la barra de cerca que apareix a la part superior. Així doncs, ens situem en aquesta i busquem el paràmetre `Ltex:LanguageTool`, per configurar *Ltex: Language Tool Http Server Uri*, que en principi no tindrà valor.
...
![Preferències d'LTeX](img/ltex3.png)
...
![Selecció d'idioma](img/ltex4.png)
